...
- **1.18.0 shipped without a rebuilt frontend bundle.** Version-pinned bundle filename (`bundle.<version>.js`): the dist held `bundle.1.17.1.js` but `index.html` requested `bundle.1.18.0.js` → 404 → blank screen. Rebuilt so filename matches version. Version bumps must run `npm run build`, not `npm run sync`.
...
- **Null content no longer crashes suggestion generation** — `generate_suggestions()` called `.strip()` directly on `response.choices[0].message.content`, which is `None` when the suggestion model (e.g. kimi-k2.6) returns reasoning-only output, refuses, or truncates on a very large prompt. The whole run threw `AttributeError: 'NoneType' object has no attribute 'strip'` and the UI showed `J is null`. Now guards both empty `choices` and null `content`, and returns a clear error suggesting the user uncheck resource types to shrink the context.
